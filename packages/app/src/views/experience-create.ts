...
export class ExperienceCreateElement extends View<Model, Msg> {
    ...
    handleRatingInput(type: string, event: Event) {
        ...
        // Ensure value is between 0 and 5 and round to 1 decimal
        ...
        if (type === "overallRating") {
            ...
            this.experience.ratings[type] = value;
    ...
    async handleSubmit() {
        const { title, category, location, description, overallRating, ratings, comment } = this.experience || {};
        const user = this.model.user;
        if (!user?.username || !user?.id || !user?.name) {
            alert("User details are missing. Please log in.");
            return;
        }
        if (!title?.trim() || !category?.trim() || !location?.trim() || !description?.trim()) {
            alert("All fields (title, category, location, description) are required.");
            return;
        }
        if (overallRating <= 0 || ratings.valueForMoney <= 0 || ratings.accessibility <= 0 || ratings.uniqueness <= 0) {
        ...
        // Dispatch message to create the initial review
        this.dispatchMessage([
            "review/create", {
                id: reviewId,
